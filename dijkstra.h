...
/* int* dijkstraWithHeap(Graph &G, int s, int t) {
    initParam(G, true);
    // heap = new MaxHeap(&G, bw);
    ...
    while(currNode != nullptr) {
        ...
        heap->insert(currNode->id);

        currNode = currNode->next;
    }

    int fringeVertex = findLargetFringeWithHeap();
    while(fringeVertex != -1) {
        status[fringeVertex] = IN_TREE;
        heap->deleteNode(fringeVertex);
        currNode = G.list[fringeVertex]->next;
        while(currNode != nullptr) {
            int currentVertexId = currNode->id;
            int newBW = min(bw[fringeVertex], currNode->weight);
            ...
            if(status[currentVertexId] == UNSEEN) {
                ...
                parent[currentVertexId] = fringeVertex;
                bw[currentVertexId] = newBW;
                heap->insert(currentVertexId);
            } else if(status[currentVertexId] == FRINGE && bw[currentVertexId] <newBW) {
                //TODO Can this be replaced with a single heapify after the change??
                heap->deleteNode(currentVertexId);
                parent[currentVertexId] = fringeVertex;
                bw[currentVertexId] = newBW;
                heap->insert(currentVertexId);
            }
        ...
        fringeVertex = findLargetFringeWithHeap();
        // cout<<"Largest fringe = "<<fringeVertex<<endl;
    }
